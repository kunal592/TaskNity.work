...
*   **User Profiles:** Individual profile pages displaying user details, assigned tasks, and performance metrics.
*   **Team & Hierarchy Management:** An organizational chart to visualize team structure and a dedicated section for managing team members (add/terminate).
*   **Leaderboard:** A "Top Contributors" panel that ranks users based on the number of completed tasks.
*   **Attendance & Leave Management:** Mark daily attendance, request leaves, and view a company-wide status board with leave reasons and approval status. Includes a historical activity graph.
*   **Finance & Expense Tracking:** Modules for tracking company expenses, managing reimbursement requests, and generating invoices with PDF support.
...
*   `POST /api/attendance`: Mark attendance for the current user (e.g., Present or Remote).
*   `GET /api/leaves`: Get all leave requests for the organization (Admin only).
*   `POST /api/leaves`: Submit a new leave request for the current user.
*   `PUT /api/leaves/{leaveId}`: Approve or reject a leave request (Admin only).
